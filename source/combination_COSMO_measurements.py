...
lastdate = '20200722120000' # !! define end date + midnight/noon
...
while firstobj != lastobj: 
    ...
    RM = average_RS_to_INCA_grid(firstobj, lastobj, INCA_grid_payerne, RM, RM)[::-1]
    ...
    # < temperature d >
    T_d_RA_1 = RA.temperature_d_mean
    ...
    #:::::::::::ABSOLUTE:::::::::::
    # < temperature >
    RA_std_temp_1 = pd.read_csv('/data/COALITION2/PicturesSatellite/results_NAL/Std_files/std_RA_temp_'+str(DT)+'.csv')
    RA_std_temp_1[RA_std_temp_1 >= 5] = np.nan
    ...
    # < temperature d >
    RA_std_temp_d_1 = pd.read_csv('/data/COALITION2/PicturesSatellite/results_NAL/Std_files/std_RA_temp_d_'+str(DT)+'.csv')
    ...
    #RM_std_temp_1[RM_std_temp_1 >= 1] = np.nan
    ...
    # RADIOMETER
    ############################################################   
    WEIGHT_RM_temp = STD_RM_temp_total / (STD_RA_temp_total + STD_COSMO_temp_total + STD_RM_temp_total)
    ...
    cmap = cm.Spectral_r
    cmap.set_bad('red',0)
    #im = ax.contourf(np.arange(0, 640), INCA_grid_payerne, STD_RM_temp_total_1[:, :, indexes[1,0]], cmap = cm.Spectral_r, levels = np.arange(0.1, 10,0.2)) # in y direction
    im = ax.contourf(np.arange(0, 640), INCA_grid_payerne, WEIGHT_RM_temp[:, :, indexes[1,0]], cmap = cm.Spectral_r, levels = np.arange(0,1,0.1)) # in y direction
